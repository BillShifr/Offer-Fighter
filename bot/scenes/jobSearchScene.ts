...
async function showProfessionalAreas(ctx: JobSearchContext, page: number) {
    try {
        ...
        if (totalPages > 1) {
            if (page > 1) {
                paginationButtons.push(Markup.button.callback('⬅️ Назад', `profarea_page_${page - 1}`));
            }
            if (page < totalPages) {
                paginationButtons.push(Markup.button.callback('Вперед ➡️', `profarea_page_${page + 1}`));
            }
        }

        const keyboard = Markup.inlineKeyboard([
            ...areaOptions.map(option =>
                [Markup.button.callback(option.name, `select_profarea_${option.id}`)]
            ),
            paginationButtons.length > 0 ? paginationButtons : [],
            [Markup.button.callback('❌ Не важно', 'select_profarea_ANY')]
        ]);
